...
private:
    QPropertyAnimation *animation_;  // 动画框架
    int duration_;                   // 动画持续时间
    bool isAnimation_;               // 是否正在动画
    QVariant currentValue_;          // 被动画改变的值
    int widgetCount_;                // StackWidget中的成员数
    int nextIndex_;                  // 下一个要切换的索引
